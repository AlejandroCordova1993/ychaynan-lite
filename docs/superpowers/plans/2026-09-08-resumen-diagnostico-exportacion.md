...
- [ ] Implementar `diagnosticReport.ts` hasta poner las pruebas en verde, reutilizando explícitamente `listAppliedAssessments`, `mapAccessState`, `parseEvaluationResult` y `adjustmentsSchema` en vez de reimplementarlos.
- [ ] Ejecutar `npx vitest run src/lib/api/diagnosticReport.test.ts src/lib/api/submissions.test.ts` (el segundo confirma que no se rompió nada reutilizado).
